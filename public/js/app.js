class VideoChat {
  ...
  async createPeerConnection() {
    ...
    this.peerConnection.onconnectionstatechange = () => {
      ...
      switch (state) {
        ...
        case "failed":
          this.isConnected = false;
          this.showNotification("Connection failed - trying again");
          this.resetConnection();
          setTimeout(() => this.findNewPartner(), 2000);
          break;
        case "connecting":
          this.showConnectionStatus("Connecting to partner...", true);
          break;
      }
    };
  }
  ...
  showConnectionStatus(message, showSpinner = false) {
    if (this.isConnected) return; // Don't show status if already connected
