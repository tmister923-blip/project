class VideoChat {
  ...
  async getUserMedia() {
    try {
      ...
      // Apply additional video enhancements via CSS filters
      this.userVideo.style.filter =
        "brightness(1.1) contrast(1.05) saturate(1.1)";
    } catch (error) {
      console.error("Error accessing media:", error);
      // Fallback with basic constraints if advanced ones fail
      try {
        ...
        this.userVideo.srcObject = this.localStream;
        this.userVideo.style.filter = "brightness(1.1) contrast(1.05)";
      } catch (fallbackError) {
        throw new Error(
          "Could not access camera/microphone. Please check permissions.",
        );
  ...
  async createPeerConnection() {
    ...
    this.peerConnection.onconnectionstatechange = () => {
      ...
      switch (state) {
        ...
        case "failed":
          this.isConnected = false;
          this.showNotification("Connection failed - searching again");
          this.resetConnection();
          setTimeout(() => this.findNewPartner(), 1000);
          break;
        case "connecting":
  ...
  async rotateCamera() {
    ...
    try {
      ...
      // Get new stream with different facing mode and enhanced quality
      ...
      // Update local video with enhanced styling
      ...
      this.showNotification("Could not rotate camera");
    }
  }
  ...
  flipCamera() {
    ...
    this.showNotification("Camera flipped");
  }
  ...
  sendMessage() {
    ...
    if (!message || !this.currentRoom || !this.isConnected) {
      if (!this.isConnected) {
        this.showNotification("Not connected to anyone yet");
      }
      return;
    }
