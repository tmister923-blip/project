class VideoChat {
  ...
  showCelebrationNotification(message) {
    ...
    setTimeout(() => {
      // Add temporary glow to the logo if visible
      const logo = document.querySelector(".animated-logo, .mini-logo");
      if (logo) {
        logo.style.animation =
          "logoFloat 0.5s ease-in-out 3, logoGlow 0.5s ease-in-out 3";
        setTimeout(() => {
          logo.style.animation = "";
        }, 1500);
      }
