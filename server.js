...
// Function to get country from IP (using a free API with better detection)
const getCountryFromIP = async (ip) => {
  try {
    // Remove IPv6 prefix if present
    const cleanIP = ip.replace(/^::ffff:/, "");

    // For development, still show local but try to get real IP
    if (
      cleanIP === "127.0.0.1" ||
      cleanIP === "::1" ||
      ...
      try {
        ...
        if (publicIpData.ip) {
          return await getCountryFromIP(publicIpData.ip);
        }
      } catch (error) {
    ...
    try {
      ...
      if (data.status === "success" && data.country) {
        // Enhanced flag mapping for more countries
        const flagMap = {
          US: "🇺🇸",
          CA: "🇨🇦",
          GB: "🇬🇧",
    ...
    try {
      response = await fetch(`https://ipinfo.io/${cleanIP}/json`, {
        timeout: 5000,
      });
      data = await response.json();
